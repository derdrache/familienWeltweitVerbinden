...
class CountryOverviewInformationPage extends StatefulWidget {
  var ortName;
  var fromCityPage;
  ...
  @override
  State<CountryOverviewInformationPage> createState() => _CountryOverviewInformationPageState();
}

class _CountryOverviewInformationPageState extends State<CountryOverviewInformationPage> {
  ...
  createNewChatGroup() async {
    ...
    var checkChatGroup = await ChatGroupsDatabase().getChatData(
        "id", "WHERE connected = '</stadt=$locationId'");
    if (checkChatGroup != false) return;

    var newChatId = await ChatGroupsDatabase().addNewChatGroup(null, "</stadt=$locationId");
...
class _CountryInformationPageState extends State<CountryInformationPage> {
  ...
  refreshCityUserInfo() {
    ...
    for (var city in stadtinfoUserData) {
      if (widget.location["ort"].contains(city["ort"])) {
  ...
  showFamilyVisitWindow() {
    ...
    for (var family in widget.location["familien"]) {
              ...
              child: Row(
                children: [
                  Icon(
                    ...
                    color: setCostIconColor(widget.location["kosten"]),
                  ),
                  const SizedBox(width: 5),
                  const Text("Kosten: "),
                  const SizedBox(width: 5),
                  Text("\$ " * widget.location["kosten"])
                ],
              ),
            ),
              ...
              child: Row(
                children: [
                  Icon(Icons.network_check_outlined,
                      color: setInternetIconColor(widget.location["internet"])),
                  const SizedBox(width: 5),
                  const Text("Internet: "),
                  const SizedBox(width: 5),
